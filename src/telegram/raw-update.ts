...
import {
  TelegramPayloadValidationError,
  buildInvalidTelegramPayloadDetails,
} from "./errors.js";
import type {
  TelegramNormalizedUpdate,
  TelegramRawUpdateEnvelope,
  TelegramRawUpdateMetadata,
  TelegramRawUpdateType,
  TelegramRequestContext,
  TelegramSupportedCommand,
} from "./types.js";
...
function normalizeRawTelegramIdentifier(value: unknown): string | undefined {
...
function detectRawUpdateType(update: Record<string, unknown>): TelegramRawUpdateType {
  const supportedTypes: TelegramRawUpdateType[] = [
    "message",
    "edited_message",
    "channel_post",
    "edited_channel_post",
...
function normalizeTelegramCommand(text: string): TelegramSupportedCommand | string | undefined {
  if (!text.startsWith("/")) {
    return undefined;
  }
...
function assertRecord(
  ...
  throw new TelegramPayloadValidationError(
    buildInvalidTelegramPayloadDetails(reason, {
      field,
      updateType,
      ...requestContext,
    }),
...
function buildMessageLikeUpdate(
  rawUpdateType: TelegramRawUpdateType,
  messageValue: unknown,
  requestContext: TelegramRequestContext,
): TelegramNormalizedUpdate {
  const message = assertRecord(
    messageValue,
  ...
  if (!chatId) {
    throw new TelegramPayloadValidationError(
      buildInvalidTelegramPayloadDetails("telegram_chat_id_missing", {
  ...
  if ("text" in message && typeof message.text !== "string") {
    throw new TelegramPayloadValidationError(
      buildInvalidTelegramPayloadDetails("telegram_message_text_invalid", {
...
function buildCallbackQueryUpdate(
  callbackQueryValue: unknown,
  requestContext: TelegramRequestContext,
): TelegramNormalizedUpdate {
  const callbackQuery = assertRecord(
    callbackQueryValue,
    "telegram_callback_query_missing",
    requestContext,
    "callback_query",
    "callback_query",
  );
  ...
  if (typeof callbackQuery.id !== "string" || callbackQuery.id.length === 0) {
    throw new TelegramPayloadValidationError(
      buildInvalidTelegramPayloadDetails("telegram_callback_query_id_missing", {
...
function buildUnsupportedUpdate(
  rawUpdateType: TelegramRawUpdateType,
...
export function parseTelegramRawUpdateEnvelope(
  rawBody: string,
  requestContext: TelegramRequestContext = {},
): TelegramRawUpdateEnvelope {
  let parsedUpdate: unknown;

  try {
    parsedUpdate = parse(rawBody);
  } catch (error) {
    throw new TelegramPayloadValidationError(
      buildInvalidTelegramPayloadDetails("telegram_payload_invalid_json", requestContext),
      error,
    );
  }
  ...
  if (!isRecord(parsedUpdate)) {
    throw new TelegramPayloadValidationError(
      buildInvalidTelegramPayloadDetails("telegram_payload_not_object", requestContext),
    );
  }
 ...
 */
export function extractTelegramRawUpdateMetadata(rawBody: string): TelegramRawUpdateMetadata {
  try {
