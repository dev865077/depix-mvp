 ...
 */
import { EulenApiError, EulenPayloadValidationError, buildInvalidEulenPayloadDetails, getEulenDepositStatus, resolveEulenDepositStatusResponse, } from "../clients/eulen-client.js";
import { createDepositEvent, listDepositEventsByDepositEntryId } from "../db/repositories/deposit-events-repository.js";
import { getDepositByDepositEntryId, getDepositByQrId, listDepositsNeedingQrIdReconciliation, updateDepositByDepositEntryId, } from "../db/repositories/deposits-repository.js";
...
function readOptionalStringField(payload, field, source, requestId) {
...
function buildWebhookPayloadErrorResult(message, reason, requestId, field) {
    return {
        ok: false,
        status: 400,
 ...
 */
export function parseEulenWebhookPayload(rawBody, requestId) {
 ...
 */
export function normalizeEulenDepositPayload(payload, requestId) {
    return {
 ...
 */
export function normalizeEulenDepositStatusPayload(payload, requestId) {
...
export function findDuplicateWebhookEvent(savedEvents, incomingEvent) {
    return savedEvents.find((savedEvent) => (savedEvent.source === WEBHOOK_SOURCE
...
export async function applyWebhookTruthToAggregate(db, tenantId, deposit, order, externalStatus) {
    ...
    const updatedOrder = await updateOrderById(db, tenantId, deposit.orderId, reconcileOrderPatch(order, mapOrderPatchFromExternalStatus(externalStatus)));
    return {
        updatedDeposit,
        updatedOrder,
    };
...
async function hydrateMissingQrIdForWebhook(input) {
    ...
    for (const candidate of candidates) {
        ...
        if (existingDepositWithQrId
            && existingDepositWithQrId.depositEntryId !== candidate.depositEntryId) {
            throw new DepositCorrelationError("deposit_qr_id_conflict", "qrId returned by Eulen is already attached to another local deposit.", {
...
export async function processEulenDepositWebhook(input) {
    ...
    if (!payload.qrId) {
        return buildWebhookPayloadErrorResult("Webhook payload must include qrId and status.", "missing_required_string", input.requestId, "qrId");
    }
    if (!payload.status) {
        return buildWebhookPayloadErrorResult("Webhook payload must include qrId and status.", "missing_required_string", input.requestId, "status");
    }
    ...
    try {
        ...
        if (repairedAggregate) {
            ...
            ({ updatedDeposit, updatedOrder } = await applyWebhookTruthToAggregate(input.db, input.tenant.tenantId, currentDeposit, currentOrder, payload.status));
        }
        log(input.runtimeConfig, {
            level: "info",
    ...
    const { updatedDeposit, updatedOrder } = await applyWebhookTruthToAggregate(input.db, input.tenant.tenantId, deposit, order, payload.status);
    log(input.runtimeConfig, {
        level: "info",
