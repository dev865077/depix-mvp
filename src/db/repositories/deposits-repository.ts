...
type PersistedDepositPatch = DepositPatch & {
  updatedAt?: string;
};
...
export class DepositOrderUniquenessError extends Error {
  ...
  tenantId: string;
  orderId: string;
  existingDeposit: DepositRecord | null;
  details: {
    tenantId: string;
    orderId: string;
    existingDepositEntryId: string | null;
    indexName: string;
  };
   ...
   */
  constructor(input: { tenantId: string; orderId: string; existingDeposit?: DepositRecord | null; cause?: unknown }) {
 ...
 */
function isDepositOrderUniquenessViolation(error: unknown): boolean {
  const message = String((error as { message?: unknown } | null | undefined)?.message ?? error ?? "").toLowerCase();
...
} as const satisfies Readonly<Record<keyof PersistedDepositPatch, string>>;
 ...
 */
export async function createDeposit(db: D1Database, input: CreateDepositInput): Promise<DepositRecord | undefined> {
  ...
  try {
    const [, selectResult] = await db.batch<DepositRecord>([insertStatement, selectStatement]);
 ...
 */
export async function getDepositByDepositEntryId(
  db: D1Database,
  tenantId: string,
 ...
 */
export function buildSelectDepositByDepositEntryIdStatement(
  db: D1Database,
  tenantId: string,
  depositEntryId: string,
): D1PreparedStatement {
 ...
 * @returns {Promise<DepositRecord | null>} Deposito encontrado.
 */
export async function getDepositByQrId(db: D1Database, tenantId: string, qrId: string): Promise<DepositRecord | null> {
  return db.prepare(`${DEPOSIT_SELECT_SQL} WHERE tenant_id = ? AND qr_id = ? LIMIT 1`).bind(tenantId, qrId).first<DepositRecord>();
}
 ...
 */
export async function getLatestDepositByOrderId(
  db: D1Database,
  tenantId: string,
  orderId: string,
): Promise<DepositRecord | null> {
  return db.prepare(
    `${DEPOSIT_SELECT_SQL}
     ...
     LIMIT 1`,
  ).bind(tenantId, orderId).first<DepositRecord>();
}
 ...
 */
export async function listDepositsNeedingQrIdReconciliation(
  db: D1Database,
  tenantId: string,
): Promise<DepositRecord[]> {
  const result = await db.prepare(
    `${DEPOSIT_SELECT_SQL} WHERE tenant_id = ? AND (qr_id IS NULL OR qr_id = deposit_entry_id) ORDER BY created_at DESC`,
  ).bind(
    tenantId,
  ).all<DepositRecord>();
 ...
 */
export async function updateDepositByDepositEntryId(
  db: D1Database,
  tenantId: string,
  depositEntryId: string,
  patch: DepositPatch,
  ...
  const [, selectResult] = await db.batch<DepositRecord>([updateStatement, selectStatement]);
 ...
 */
export function buildUpdateDepositByDepositEntryIdStatement(
  db: D1Database,
  tenantId: string,
  depositEntryId: string,
  patch: DepositPatch,
): D1PreparedStatement | null {
  const patchEntries = getAllowedPatchEntries<PersistedDepositPatch>(
    {
      ...patch,
