...
import telegramConfirmation from "../src/services/telegram-order-confirmation.ts?raw";
...
describe("financial API boundary contract", () => {
  ...
  it("stays grounded in the current monolith surface and direct coupling points", function assertCurrentRuntimeEvidence() {
    ...
    expect(telegramRuntime).toContain("confirmTelegramOrder");
    expect(telegramRuntime).toContain("processDepositRecheck");
    expect(telegramConfirmation).toContain("createEulenDeposit(");

    expect(contract).toContain("Bot payment creation still calls Eulen directly");
    expect(contract).toContain("Bot pending-payment status still triggers direct reconciliation");
